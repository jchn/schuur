...
const removeEntityFromStore = (store, entityModel, entityId) => {
    ...
    if (relation.type === 'hasMany') {
      ...
      if (relationWithRelatedEntity === 'belongsTo') {
        relatedEntities.forEach(relatedEntity => {
          store = updateEntity(store, relation.model, relatedEntity.id, {[entityModel.type]: null})
        })
      }
      ...
      if (relationWithRelatedEntity === 'hasMany') {
        relatedEntities.forEach(relatedEntity => {
          store = updateEntity(store, relation.model, relatedEntity.id, {[entityModel.typePlural]: relatedEntity[entityModel.typePlural].filter(id => id !== entityId)})
        })
      }
    }
